<template>
  <div class="qus">
    <p>Subscription depends on:</p>
    <p>- number of patients ?</p>
    <p>- number of doctors ?</p>
    <p>- duration</p>
    <p>- .. ?</p>
    <p>****************</p>
    <p>Subscription Description:</p>
    <p>- date of purchase</p>
    <p>- duration</p>
    <p>- number max of patients per plan</p>
    <p>- number max doctors per plan</p>
    <p>- list of people using the subscription</p>
    <p>- Who pay it: the doctor , the hospital admin or patient ?</p>
    <p>- will send invoice to customers ?</p>
    <p>Does it have a trial time ?</p>
    <p>What are the different possible plans ?</p>
    <p>What are the payment methods? Is it Per month or year ?</p>

  </div>
</template>

<script>
export default {
  data() {
    return {};
  },
  created() {},
  mounted() {},
  methods: {}
};
</script>

<style  scoped>
.qus {
  background-color: white;
}
</style>
